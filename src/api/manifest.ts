...
export interface LightManifest {
  ...
  DestinyInventoryBucketDefinition: Record<number, DestinyInventoryBucketDefinition>;
  ...
  DestinyPresentationNodeDefinition: Record<number, DestinyPresentationNodeDefinition>;
}
...
export async function loadLightManifest(locale = "fr"): Promise<LightManifest> {
  purgeLegacyCaches();
  const info = await bungieGet<ManifestInfo>("/Destiny2/Manifest/", { auth: false });
