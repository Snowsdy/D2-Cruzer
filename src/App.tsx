...
const Activities = lazy(() =>
  import("@/pages/activities/Activities").then((m) => ({
    default: m.Activities,
  }))
)
const Database = lazy(() =>
...
const PlaytimeView = lazy(() =>
  import("@/pages/tools/PlaytimeView").then((m) => ({
    default: m.PlaytimeView,
  }))
)
...
const NightfallView = lazy(() =>
  import("@/pages/tools/NightfallView").then((m) => ({
    default: m.NightfallView,
  }))
)
const IronBannerView = lazy(() =>
  import("@/pages/tools/IronBannerView").then((m) => ({
    default: m.IronBannerView,
  }))
)

function RouteFallback() {
  return (
    <div className="text-bungie-muted flex items-center justify-center py-20 text-sm">
      Chargement…
    </div>
  )
}
    ...
    <>
      ...
      <Routes>
        ...
        >
          ...
          <Route
            path="loadouts"
            element={<Navigate to="/inventory/loadouts" replace />}
          />
          {/* Legacy: rolls + armor moved inside Inventory */}
          <Route
            path="rolls"
            element={<Navigate to="/inventory/rolls" replace />}
          />
          <Route
            path="armor"
            element={<Navigate to="/inventory/armor" replace />}
          />
