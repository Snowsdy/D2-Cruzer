...
export function PlatformSwitcher() {
  ...
  return (
    <div
      className="flex items-center gap-1 bg-bungie-panel/60 border border-bungie-border rounded-full p-1 shrink-0"
      ...
      {list.map((m) => {
        ...
        return (
          <button
            ...
            className={`h-7 px-2.5 rounded-full text-[10px] uppercase tracking-widest font-bold transition-all flex items-center gap-1.5 ${
              active
                ? "bg-bungie-accent/25 text-bungie-accent border border-bungie-accent/50"
                : "text-bungie-muted hover:text-white hover:bg-white/5"
            }`}
          >
            <span className="text-xs leading-none">{platformIcon(m.membershipType)}</span>
            <span>{platformLabel(m.membershipType)}</span>
            {isCrossSaveOwner && (
              <span
                className={`text-[8px] px-1 rounded ${active ? "bg-bungie-accent/30" : "bg-white/10"}`}
