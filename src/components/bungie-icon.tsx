...
export function BungieIcon({
  ...
  if (source === "item") {
    ...
    icon = dp?.smallTransparentIcon || dp?.mapIcon || dp?.icon
  } else if (manifest) {
    switch (source) {
      case "category":
        icon =
          manifest.DestinyItemCategoryDefinition?.[hash]?.displayProperties
            ?.icon
        break
      case "bucket":
        icon =
        ...
        break
      case "stat":
        icon = manifest.DestinyStatDefinition?.[hash]?.displayProperties?.icon
        break
      case "damage":
        icon =
        ...
        break
      case "class":
        icon = manifest.DestinyClassDefinition?.[hash]?.displayProperties?.icon
        break
      case "activityMode":
        icon =
      ...
      const present =
        manifest && source !== "item"
          ? !!(
              ...
              (source === "bucket" &&
                manifest.DestinyInventoryBucketDefinition?.[hash]) ||
              (source === "stat" && manifest.DestinyStatDefinition?.[hash]) ||
              (source === "damage" &&
                manifest.DestinyDamageTypeDefinition?.[hash]) ||
              (source === "category" &&
                manifest.DestinyItemCategoryDefinition?.[hash])
            )
          : null
