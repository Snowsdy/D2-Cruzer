...
export function LivePlayersBadge() {
  ...
  return (
    <Link
      to="/tools/stats"
      title={title}
      className="bg-bungie-panel/50 flex h-7 items-center gap-1.5 rounded-full border border-emerald-400/25 px-2 transition-colors hover:border-emerald-400/60"
    >
      <span className="relative h-1.5 w-1.5 shrink-0">
        ...
        />
        {total != null && (
          <span className="absolute inset-0 h-1.5 w-1.5 animate-ping rounded-full bg-emerald-400 opacity-50" />
        )}
      </span>
      <span className="text-[11.5px] leading-none font-bold text-white tabular-nums">
        {total != null ? formatCompact(total) : q.isLoading ? "…" : "—"}
      </span>
    </Link>
  )
}
