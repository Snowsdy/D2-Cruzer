...
export function Dropdown<T extends string>({
  ...
  const position = useCallback(() => {
    ...
    const panelH = Math.min(
      320,
      Math.max(44, options.length * 34 + 8)
    );
  ...
  return (
    <>
      <button
        ...
        disabled={disabled}
        className={[
          "inline-flex items-center gap-2 bg-bungie-panel/60 border text-white font-semibold uppercase tracking-wider appearance-none cursor-pointer transition-colors",
          SIZES[size],
          SHAPES[variant],
          open
      ...
      >
        {icon && (
          <span className="inline-flex items-center justify-center w-3.5 h-3.5 text-bungie-accent/80 shrink-0">
            {icon}
          </span>
        )}
        ...
        <svg
          ...
          className={`shrink-0 ml-auto opacity-70 transition-transform ${open ? "rotate-180" : ""}`}
        >
          <path d="M6 9l6 6 6-6" />
        </svg>
      </button>
      ...
      {open &&
        ...
        createPortal(
          <div
            ...
            className={[
              "overflow-auto rounded-xl border border-bungie-accent/30 backdrop-blur-xl p-1",
            ...
            {options.map((o, i) => {
              ...
              return (
                <button
                  ...
                  disabled={o.disabled}
                  className={[
                    "w-full flex items-center gap-2 px-2.5 h-8 rounded-lg text-[12px] font-semibold text-left transition-colors",
                    o.disabled
