  ...
  return useMemo<SeasonInfo | null>(() => {
    ...
    const progress =
      startDate && endDate && endDate.getTime() > startDate.getTime()
        ? Math.min(
            1,
            Math.max(0, (now - startDate.getTime()) / (endDate.getTime() - startDate.getTime()))
          )
        : 0;
    ...
    if (seasonDefExt.seasonPassHash) candidatePassHashes.push(seasonDefExt.seasonPassHash);
    ...
    let seasonPassDef: { rewardProgressionHash?: number; prestigeProgressionHash?: number } | undefined;
    ...
    if (!seasonPassDef) {
      ...
      const keys = Object.keys(allPasses).map(Number).filter((n) => !isNaN(n));
      if (keys.length > 0) {
        keys.sort((a, b) => b - a);
        for (const h of keys) {
    ...
    const orderedCharIds = [
      ...(activeCharacterId && charIds.includes(activeCharacterId) ? [activeCharacterId] : []),
      ...charIds.filter((c) => c !== activeCharacterId),
    ];

    let bestRewardProg: { level: number; progressToNextLevel: number; nextLevelAt: number } | undefined;
    let bestPrestigeProg: { level: number; progressToNextLevel: number; nextLevelAt: number } | undefined;
    ...
    if (import.meta.env.DEV && rewards.length === 0) {
      console.warn("[season] no rewards loaded", {
        ...
        hasRewardProgDef: !!manifest.DestinyProgressionDefinition?.[rewardProgHash],
        hasPrestigeProgDef: !!manifest.DestinyProgressionDefinition?.[prestigeProgHash],
        manifestProgDefCount: Object.keys(manifest.DestinyProgressionDefinition ?? {}).length,
        rewardLevel,
        prestigeLevel,
        charIds,
      });
    ...
    return {
      hash: seasonHash,
      name: seasonDef.displayProperties?.name ?? `Saison ${seasonDef.seasonNumber}`,
 ...
 */
export function usePastSeasonDetail(hash: number | null): PastSeasonDetail | null {
  const manifest = useManifestStore((s) => s.manifest);
...
type ManifestShape = NonNullable<ReturnType<typeof useManifestStore.getState>["manifest"]>;
...
function collectSeasonRewards(
  ...
  if (ext.seasonPassProgressionHash) freeProgHashes.add(ext.seasonPassProgressionHash);
  if (ext.rewardProgressionHash) freeProgHashes.add(ext.rewardProgressionHash);
  if (ext.prestigeProgressionHash) premiumProgHashes.add(ext.prestigeProgressionHash);
  ...
  for (const passHash of passHashes) {
    ...
    if (!passDef) continue;
    if (passDef.rewardProgressionHash) freeProgHashes.add(passDef.rewardProgressionHash);
    if (passDef.prestigeProgressionHash) premiumProgHashes.add(passDef.prestigeProgressionHash);
  }
