...
function computeMilestoneState(
  ...
  if (activities.length > 0) {
    const allDone = activities.every((a) => {
      ...
      const phasesOk =
        phases.length === 0 || phases.every((p) => p.complete);
      const challengesOk =
        challenges.length === 0 ||
...
export function Checklist() {
  ...
  const splitPursuits = useMemo(() => {
    ...
    const cmp = (
      ...
      if (sort === "name") return nameOf(a.itemHash).localeCompare(nameOf(b.itemHash));
    ...
    for (const cid of pursuitCharIds) {
      ...
      bounties[cid] = list
        .filter((p) => pursuitTypeByHash.get(p.itemHash) === ITEM_TYPE.Bounty && filter(p))
        .sort(cmp);
      quests[cid] = list
        .filter((p) => pursuitTypeByHash.get(p.itemHash) !== ITEM_TYPE.Bounty && filter(p))
        .sort(cmp);
  ...
  const recordState = (r: DestinyRecordComponent): "claim" | "progress" | "done" | "hide" => {
    const s = r.state ?? 0;
    ...
    const hasProgress = (r.objectives ?? []).some(
      (o) => (o.progress ?? 0) > 0
    );
  ...
  const actionableRecords = useMemo(() => {
    const list: Array<{ hash: number; rec: DestinyRecordComponent; state: "claim" | "progress" | "done" }> = [];
  ...
  const loading =
    milestones.isLoading || defs.some((q) => q.isLoading);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Reset timers */}
      <div className="stagger grid grid-cols-1 md:grid-cols-2 gap-3">
        <div className={`panel p-4 border ${ACCENT_BORDER}`}>
          ...
          <div className="text-[10px] text-bungie-muted mt-1">
        ...
        <div className="panel p-4 border border-bungie-border">
      ...
      {/* Tab selector */}
      <div className="flex gap-1 p-1 bg-black/30 border border-bungie-border rounded-full self-start w-fit">
        {(
          [
            { k: "milestones", label: t("checklist.tabMilestones"), count: allVisible.length },
            { k: "bounties", label: t("checklist.tabBounties"), count: totalBounties },
            { k: "quests", label: t("checklist.tabQuests"), count: totalQuests },
            { k: "triumphs", label: t("checklist.tabTriumphs"), count: totalTriumphs },
          ] as const
        ).map((x) => {
  ...
  return (
    <section>
      <h2 className="text-lg font-semibold mb-3 flex items-center gap-2">
        ...
        <span>{title}</span>
        <span className="text-xs text-bungie-muted">({items.length})</span>
      </h2>
...
function MilestoneCard({
  ...
  return (
    <div
      className="relative panel overflow-hidden p-4 min-h-40 flex flex-col"
      style={
        bannerPath
          ? {
    ...
    >
      <div className="flex items-start gap-3">
        {icon && (
          <img
            ...
            alt=""
            className="w-10 h-10 rounded border border-white/20 bg-black/50 shrink-0"
          />
        )}
        <div className="flex-1 min-w-0">
          <div className="font-bold text-white drop-shadow leading-tight">
            {name}
          </div>
          {endHint && (
            <div className="text-[10px] uppercase tracking-widest text-amber-300/80 mt-0.5">
              {endHint}
            </div>
          )}
        </div>
        <span
          className={`text-[9px] uppercase tracking-widest font-bold px-1.5 py-0.5 rounded border shrink-0 ${stateBadge.cls}`}
        >
          {stateBadge.label}
        </span>
      </div>
      ...
      {description && (
        <p className="text-xs text-white/75 mt-3 line-clamp-3">
          {description}
        </p>
      )}
      ...
      {activityInfos.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-auto pt-3">
          {activityInfos.slice(0, 3).map((a) => (
            <span
              key={a.hash}
              className="text-[10px] uppercase tracking-widest font-semibold px-2 py-0.5 rounded-full border border-pink-400/40 bg-pink-400/10 text-pink-200 truncate max-w-50"
              title={a.name}
            >
              {a.name ?? `#${a.hash}`}
              {a.modifiers > 0 && (
          ...
          {activityInfos.length > 3 && (
            <span className="text-[10px] text-bungie-muted self-center">
              +{activityInfos.length - 3}
            </span>
          )}
        </div>
      ...
      {m.charStates.length > 0 && (
        <div className="flex items-center gap-1 pt-2 mt-1 border-t border-white/5">
...
function TriumphCard({
  ...
  return (
    <div
      className={`panel p-3 flex gap-3 border ${accentBorder} hover:border-white/30 transition-colors`}
    >
      {icon && (
        <img
          src={`https://www.bungie.net${icon}`}
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <span
            className={`text-[9px] uppercase tracking-widest font-bold px-1.5 py-0.5 rounded border ${accentBorder} ${accentText}`}
          >
            {item.state === "claim"
              ? "★"
              : item.state === "done"
        ...
        {description && (
          <p className="text-[11px] text-bungie-muted mt-0.5 line-clamp-2">
            {description}
          </p>
        )}
        {objectives.length > 0 && (
          <div className="mt-2 space-y-1">
            ...
            {objectives.length > 2 && (
              <div className="text-[10px] text-bungie-muted">
                +{objectives.length - 2}
              </div>
            )}
          </div>
  ...
  return (
    <section>
      <h2 className="text-lg font-semibold mb-3 flex items-center gap-2">
        ...
        <span>{title}</span>
        <span className="text-xs text-bungie-muted">({totalCount})</span>
      </h2>
      <div className="space-y-4">
        {charIds.map((cid) => {
          ...
          return (
            <div key={cid}>
              <h3 className="text-xs font-semibold mb-2 flex items-center gap-2 text-bungie-muted">
                {c?.emblemPath && (
                  <img
                    src={`https://www.bungie.net${c.emblemPath}`}
                    alt=""
...
function PursuitCard({
  ...
  return (
    <button
      ...
      className={`panel p-3 flex gap-3 border text-left cursor-pointer hover:-translate-y-0.5 transition-all ${accentBorder} hover:border-white/40`}
    >
      {icon && (
        <img
          src={`https://www.bungie.net${icon}`}
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {allComplete && (
            <span className="text-[9px] uppercase tracking-widest font-bold px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/40">
              ...
              return (
                <div key={i}>
                  <div className="flex items-center justify-between text-[10px] tabular-nums">
                    ...
                    <span className={o.complete ? "text-emerald-300" : "text-white/60"}>
                      {Math.round(pct)}%
                    </span>
                  </div>
                  <div className="h-1 bg-black/40 rounded-full overflow-hidden">
