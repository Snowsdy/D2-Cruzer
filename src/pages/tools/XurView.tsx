          ...
          {sales.map((s, i) => {
            ...
            return (
              <button
                ...
                className="panel p-4 flex gap-3 border border-bungie-border hover:border-amber-500/40 hover:-translate-y-0.5 transition-all text-left cursor-pointer"
              >
                {icon && (
                  <img
                    src={`https://www.bungie.net${icon}`}
                    alt=""
                    className="w-16 h-16 rounded-lg border border-white/20 bg-black/40 shrink-0"
                  />
                )}
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-bungie-muted truncate">
                    {typeName}
                  </div>
                  <div className="flex items-center gap-1 mt-2 flex-wrap">
                    {tier && (
                      <span
                        className={`text-[10px] uppercase tracking-widest font-semibold px-2 py-0.5 rounded-full border ${tierClass}`}
                      >
                        {tier}
                      </span>
                    )}
                  ...
                  {s.costs && s.costs.length > 0 && (
                    <div className="flex items-center gap-2 mt-2 text-xs text-white/80">
                      {s.costs.map((c, ci) => (
                        <CostBadge key={ci} itemHash={c.itemHash} quantity={c.quantity} />
                      ))}
                    </div>
                  )}
                </div>
...
function CostBadge({
  ...
  return (
    <span className="inline-flex items-center gap-1 bg-black/40 border border-white/10 rounded px-1.5 py-0.5 tabular-nums">
      {icon && (
        <img
          src={`https://www.bungie.net${icon}`}
          alt=""
