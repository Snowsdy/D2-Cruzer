...
export function PlaytimeContent() {
  ...
  const totalSeconds = useMemo(() => {
    ...
    for (const c of charsArr)
      total += (Number(c.minutesPlayedTotal) || 0) * 60;
    return total;
  }, [charsArr]);
  ...
  const modeBreakdown = useMemo(() => {
    ...
    if (!r) return [];
    const rows: Array<{ key: string; label: string; seconds: number; color: string }> = [
      {
        key: "pvp",
      ...
      <div className="panel p-5">
        ...
        <div className="stagger grid gap-3 md:grid-cols-3">
          {charsArr.map((c) => {
            ...
            const raceName =
              raceDefs[c.raceHash]?.displayProperties?.name ?? "";
            return (
              <div
                key={c.characterId}
                className="relative overflow-hidden rounded-xl border border-bungie-border bg-bungie-panel/50 p-4"
                style={
