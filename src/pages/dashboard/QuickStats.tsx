...
function StatTile({
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute -top-8 -right-8 h-24 w-24 rounded-full opacity-0 blur-2xl transition-opacity group-hover:opacity-100"
        style={{ background: accentTint[accent] }}
      />

      <div className="relative flex items-start justify-between gap-2">
        <div
          className={`flex h-9 w-9 items-center justify-center rounded-lg ${iconColor[accent]}`}
          style={{
        ...
        {tag && (
          <span className="text-[9px] font-extrabold tracking-[0.2em] text-white/40 uppercase">
            {tag}
          </span>
        )}
      </div>
      <div className="text-bungie-muted mt-3 text-[10px] font-bold tracking-[0.2em] uppercase">
        {label}
      </div>
      <div className="mt-1 text-[26px] leading-none font-extrabold text-white tabular-nums">
        {value}
        {suffix && (
          <span className="text-bungie-muted ml-1 text-sm font-semibold">
            {suffix}
          </span>
        )}
      </div>
...
export function QuickStats() {
  ...
  const totalSeconds =
    readStat(pve, "secondsPlayed") + readStat(pvp, "secondsPlayed")
