...
export function Dupes() {
  ...
  const allItems: Array<{ item: DestinyItemComponent; owner: string | null }> =
    useMemo(() => {
      ...
      for (const cid of Object.keys(chars)) {
        ...
        for (const it of profile.data?.characterInventories?.data?.[cid]
          ?.items ?? []) {
          if (it.itemInstanceId) out.push({ item: it, owner: cid })
        }
      }
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {enriched.map((g) => (
          <div
            key={g.itemHash}
            className="panel grid items-center gap-4 p-3"
            style={{
            ...
            <div className="flex min-w-0 items-center gap-3">
              {g.icon && (
                <img
                  ...
                  alt=""
                  className="border-bungie-border h-12 w-12 shrink-0 rounded border"
                />
              )}
