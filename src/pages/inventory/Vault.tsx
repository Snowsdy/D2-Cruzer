...
export function Vault() {
  ...
  }, [filter, vaultItems, manifest, query, defMap, itemStats, itemInstances, tagsState, sort]);
      ...
      <div className="flex items-baseline justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-xl font-bold inline-flex items-center gap-2">
            ...
            {t("inventory.vault")}
            <span className="text-sm text-bungie-muted font-normal tabular-nums">
          ...
          {sections.map((s) => (
            <section key={s.bucketHash} className="panel p-4">
              <div className="flex items-baseline justify-between mb-3">
                <h3 className="font-semibold text-bungie-accent tracking-wide uppercase text-sm">
                  {s.bucketName}
                </h3>
                <span className="text-xs text-bungie-muted tabular-nums">
                  {s.items.length}
                </span>
              </div>
              <div className="flex flex-wrap gap-2">
                {s.items.map((it) => (
                  <ItemTile
                    key={it.itemInstanceId ?? `${s.bucketHash}-${it.itemHash}-${Math.random()}`}
                    item={it}
                    size="md"
